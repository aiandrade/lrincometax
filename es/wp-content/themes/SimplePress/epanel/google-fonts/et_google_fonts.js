(function($){
	...
	ET_Google_Fonts.prototype = {
		...
		select_option: function(event) {
			...
			/*self.maybe_request_font( this_option_value, $this_option );

			self.apply_font( this_option_value, $this_option );*/
		...
		maybe_request_font: function( font_name, font_option ) {
			if ( font_option.data( 'value' ) === 'none' ) return;
		...
		apply_font: function( font_name, font_option ) {
			if ( font_option.data( 'value' ) === 'none' ) {
				$( "#customize-preview iframe" ).contents().find( 'style.' + this.options.used_for ).remove();

				return;
			}

			$("#customize-preview iframe").contents().find('head').append( '<style class="' + this.options.used_for + '">' + this.options.apply_font_to + ' { font-family: "' + font_name + '", sans-serif; } </style>' );
		},
